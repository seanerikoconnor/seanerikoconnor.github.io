...
bool PolyOrder::maximal_order()
{
    ...
    // Start with the polynomial x, modulo f(x) and p.
    Polynomial x1( "x", f_.modulus() ) ;
    PolyMod x( x1, f_ ) ;  

    #ifdef DEBUG_PP_SLOW_PRIMITIVITY_CHECK
    cout << endl << "Starting with x (mod f(x),p) = " << x << endl ;
    #endif

    bool x_to_k_is_one_too_early = false ;

    BigInt k { 1u } ;
    while ( k < maxOrder )
    {
        PolyMod x_to_power = power( x, k ) ; // x^k (mod f(x), p)

        #ifdef DEBUG_PP_SLOW_PRIMITIVITY_CHECK
        cout << "k =        " << k << " x^k (mod f(x),p) = " << x_to_power
             << "   x^k is integer = " << (x_to_power.isInteger() ? "yes" : "no")
             << "   x^k has constant term x[0] = " << x_to_power[0] << endl ;
        #endif

        // x^k (mod f(x), p) = 1 for k < p^n-1, so x cannot be a generator of the field.
        if (x_to_power.isInteger() && x_to_power[0] == 1u)
        {
            #ifdef DEBUG_PP_SLOW_PRIMITIVITY_CHECK
            cout << "x^k = 1 for k = " << k << " break out of the loop" << endl ;
            #endif

            x_to_k_is_one_too_early = true ;
            break ;
        }
    ...
    // Order of x < p^n-1, so definitely not primitive.
    if (x_to_k_is_one_too_early)
        return false ;
    // x^k != 1 for k = 1, ..., p^n-2.  x could be still primitive. 
    else
    {
        PolyMod x_to_power = power( x, maxOrder ) ;

        #ifdef DEBUG_PP_SLOW_PRIMITIVITY_CHECK
        cout << "maxOrder = " << maxOrder << " x^maxOrder (mod f(x),p) = " << x_to_power
             << "   x^maxOrder is integer = " << (x_to_power.isInteger() ? "yes" : "no")
             << "   x^maxOrder has constant term x[0] = " << x_to_power[0] << endl ;
        #endif

        if (x_to_power.isInteger() && x_to_power[0] == 1u)
            // x^k = 1 for k = p^n-1, so primitive by definition.
            return true ;
        else
            return false ;
    }
